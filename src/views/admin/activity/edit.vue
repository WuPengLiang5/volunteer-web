<template>
  <div class="edit-activity">
        ...
        <el-row>
          <el-col :span="15">
            <el-form-item label="活动人数上限" prop="registerMax">
              <el-input v-model="form.registerMax" type="number"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
    ...
    <Map ref="map" :width="'50%'" :dialog-visible="flag" :address="form.location"  @map-confirm="mapConfirm"  @map-cancel="mapCancel"></Map>
  </div>
</template>

<script>
import {updateActivity, fetchActivity} from "@/api/admin/activity"
...
export default {
  ...
  methods:{
    fetchData(id){
      fetchActivity({id:id}).then(resp => {
        if(resp.data.code === 200){
          this.form = resp.data.data
          this.previewImgURL = this.form.cover
    ...
    saveForm(formName) {
      this.$refs[formName].validate(async(valid) => {
        if (valid) {
          ...
          // 保存
          await updateActivity(this.form).then(resp => {
