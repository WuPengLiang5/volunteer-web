    ...
    <div class="user-info-content">
      <el-form ref="formData" :model="formData" class="formData" label-position="left" label-width="82px">
        ...
        <el-form-item
            label="性别"
            prop="gender">
          <el-radio :disabled="!isEdit" v-model="formData.gender" label="男">男</el-radio>
          <el-radio :disabled="!isEdit" v-model="formData.gender" label="女">女</el-radio>
        </el-form-item>
...
export default {
  ...
  methods:{
    fetchData(){
      fetchVolunteerInfo(this.userId).then(resp => {
        this.formData = resp.data.data
        console.log(this.formData)
      })
    },
    onSubmit(){
      // let url='/worker/updateWorker'
      // if (this.role==='3'){
      //   url='/guest/updateGuest'
      // }
      // this.$axios({
      //   url:url,
      //   method:'post',
