          ...
          <el-row v-cloak>
            <el-col :span="8" class="activitydetail-left">
              <div class="activity-item-box">
                ...
                <p class="activity-item-plist">
                  活动日期：
                  <span>{{activity.beginTime | formatDate}}</span>
                  至
                  <span>{{activity.endTime | formatDate}}</span>
                </p>
                <p class="activity-item-plist">
                  招募日期：
                  <span>{{activity.registrationBegin | formatDate}}</span>
                  至
                  <span>{{activity.registrationEnd | formatDate}}</span>
                </p>
                <p class="activity-item-plist">
...
import {fetchSignUpByActivityId, fetchSignUpByAIdAndUId, createSignUp} from "@/api/signUp";
...
export default {
  ...
  methods:{
    ...
    getList(){
      fetchSignUpByActivityId({activityId:this.activity.id}).then(resp => {
        if (resp.data.code === 200){
