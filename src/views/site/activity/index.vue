    ...
    <div class="main">
      <div class="mid">
        <div class="filter-container">
          ...
          <el-input v-model="listQuery.location" placeholder="新闻来源" style="width: 200px" class="filter-item" @keyup.enter.native="handleFilter" />
          <el-select v-model="listQuery.state" placeholder="状态" clearable class="filter-item" style="width: 130px">
            <el-option v-for="item in activityState" :key="item.key" :label="item.label" :value="item.key" />
          </el-select>
            ...
            <el-col :span="6" class="activity-item-box" v-for="item in list" :key="item.id">
              <router-link :to="{path:'/site/activityDetail',query:{id:item.id}}">
                <el-card :body-style="{padding:'0px'}" style="width: 260px" shadow="hover">
                  <el-image :src="item.cover" class="activity-item-img"></el-image>
...
export default {
  ...
  data(){
    return {
      ...
      activityState:[{key:'',label:'全部'},{key:'0',label:'招募待启动'},{key:'1',label:'招募中'},{key:'2',label:'招募已结束'},{key:'3',label:'已结束'}]
    }
  },
  methods:{
    getList(){
    ...
    changePercentage(start, end){
      ...
      let result
      if ((staytimeGap - currentStaytimeGap) <= 0){
        result = 100
      }else {
        result = Math.floor((currentStaytimeGap / staytimeGap) * 100)
      }

      // console.log(currentStayDay,stayDay,result)
      return result
    },
